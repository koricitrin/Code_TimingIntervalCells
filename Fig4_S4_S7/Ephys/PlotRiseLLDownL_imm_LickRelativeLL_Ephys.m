function PlotRiseLLDownL_imm_LickRelativeLL_Ephys(cond)
    ...
    fr_profile_path = ['Z:\Kori\immobile_code\Ephys\0403\FRprofileTable_align' CondStr 'PAC.mat'];
    PACPath = ['Z:\Kori\immobile_code\Ephys\0403\'];
     
    tablePath = ['Z:\Kori\immobile_code\Ephys\0403\'];
...
timeStepRun = timeStepRun/1250;
    ...
    [~,sorted_All_ind]  = sort(RiseLLDownLickRatio.RatioLastLick, 'ascend'); 
...
FR_Array_sorted_67 = [];
 ...
 FR_Array_sorted_67_NotNorm(any(isnan(FR_Array_sorted_67_NotNorm), 2), :) = [];
   ...
   set(gcf, 'Position', [100 100  550 522]);
    xlim([find(timeStepRun == -2), find(timeStepRun == 7)]);
   x_tick_pos = [find(timeStepRun == -2), find(timeStepRun == -1), find(timeStepRun == 0), find(timeStepRun == 1),...
                 find(timeStepRun == 2), find(timeStepRun == 3), find(timeStepRun == 4), find(timeStepRun == 5), find(timeStepRun == 6),...
                 find(timeStepRun == 7), find(timeStepRun == 8), find(timeStepRun == 9)];     
   xticks(x_tick_pos);
 ...
 print('-painters','-dpdf', [save_path1 '.pdf'], '-r600')
  ...
  ax.FontSize = 17;
    ...
    set(gca, 'ytick', 0:20:size(FR_Array_sorted_67,1));
    ylabel('Neurons');
    xlabel(namex);
    save_path1  = ['RiseLLDownL-' CondStr '67'];
    savefig([save_path1 '.fig'])
    print('-painters','-dpdf', [save_path1 '.pdf'], '-r600')
    print('-painters','-dpng', [save_path1 '.png'], '-r600')
...
savename = ['RiseDownArray4555_67' CondStr 'manual' '.mat'];
  save(savename, 'FR_Array_sorted', 'FR_Array_sorted_67', 'FR_Array_sorted_4555', 'timeStepRun')
