function RiseDownTable = IdentifyRiseDownImmEphys(path, filename, savepath, threshold)
...
timeStepRun = timeStepRun/1250;
    ...
    % indFR0to1 = timeStepRun >= 100 & timeStepRun < 1000;
    % indFRBefRun = timeStepRun >= -1000 & timeStepRun < 100; % different from cue!!
    ...
    for j = 1:length(Neurons)
        k = Neurons(j);
        fsa_j = filteredSpikeArray{k}(:,1:end);
        ...
        % mean_fsa_j = mean_fsa_temp/(max(mean_fsa_temp));   
        %  figure
        % plot(mean_fsa_j)
        ratio0to1BefRun_j = mean(mean_fsa_j(indFR0to1))/mean(mean_fsa_j(indFRBefRun));     
        fsa_j_norm = [];
        rec_name = [rec_name ; string(filename(i,1:16))];
        neu_id = [neu_id ; k];
   ...
   RiseDownTable = table(rec_name, neu_id, ratio0to1BefRun, isRise, isDown);
    ...
    %  for j = 1:length(Neurons)
    %     k = Neurons(j);
    %     fsa_j = filteredSpikeArray{k}(TrialsEven,1:end);
    %     mean_fsa_temp = mean(fsa_j,1);
    % 
    %     %normalize to get rid of neg dFF values
    %     mean_fsa_j =  (mean_fsa_temp - min(mean_fsa_temp))/(max(mean_fsa_temp) - min(mean_fsa_temp));     
    ...
    %     DownThres = 2/3; 
    %     thresStr = 'thres';
    %     isRise = [isRise ; ratio0tomak1BefRun_j > RiseThres & ~isinf(ratio0to1BefRun_j)];
    %     isDown = [isDown ; ratio0to1BefRun_j < DownThres & ~isinf(ratio0to1BefRun_j)];
    %     % mFR_j = mean(mFRStructNonStim.mFRArr(k,:));
    %     % mFR = [mFR ; mFR_j];
    %     end
...
%     save(save_path, 'RiseDownTableGood', 'RiseDownTableBad' );
   cd(save_path)
    save_path1 = [filename(i,1:16) '_RiseDownID_' CondStr thresStr '2' '.mat'];
