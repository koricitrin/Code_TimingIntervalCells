...
function PlotPACsortLick(paths, filename, PAC, savepath)
      ...
      if PAC == 1 
        ...
        NeurSel = PACmanual_NoInt;
         ...
         InfoPath = [paths_i  '_Info.mat'];
        load(InfoPath)
        AllNeur = 1:length(autoCorr.isInterneuron);
        NeurSel = AllNeur(logical(autoCorr.isInterneuron));
        cd(savepath)
        foldername = ['IndLLtoFLickPlot_INT'];  
         mkdir(foldername)  
         savepaths = [savepath foldername]
      end
        ...
        % 1:size(filteredSpikeArrayLastLickOnSet,2)
              ...
              %    % % 
               ...
               xlim([-3 10])
                   ...
                   plot(firstLickSortedEdit,tr, 'Color', [.5 0 .5], 'LineWidth',2)   %uncomment later
                  ...
                  % l = legend('First Lick')
                    ...
                    set(gca,'FontSize',17.5)
                     fileName1 = ['Neuron' num2str(b) '-' sess ]; %uncomment later
                     savefig([ fileName1 '.fig'])
                    print('-painters','-dpng',[ fileName1],'-r600');
        
              end
              clearvars -except paths filename PAC savepath
        end
        close all
end