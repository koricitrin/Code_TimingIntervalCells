...
COM_56_Cat= [];
    ...
    for k = 1:size(Paths,1)
        path = Paths(k,:);
        cd([path '\PAC_COM_EarlyLate_0403\'])
...
data67 = [mean(COM_67_Cat_Sec), sem_data_67]
  ...
  if plotnum == 1
    ...
    savefig([T '.fig'])
     ...
     subplot(2,1,1)
    hold on
    histogram(COM_4555_Cat_Sec, 'BinWidth', 0.14,  'FaceColor', 'k', 'Normalization', 'probability')
    histogram(COM_67_Cat_Sec, 'BinWidth', 0.14, 'FaceColor',  [0 0.4470 0.7410], 'Normalization', 'probability')
    ylim([0 0.4])
    xlim([0 7])
    xticks([0:7])
    % xticks([0:500:3500])
    ...
    title('Center of mass')
        subplot(2,1,2)
        hold on
        load('Z:\Kori\immobile_code\Ephys\0403\COM_Norm_Warp.mat')
    histogram(com_4555/2800, 'BinWidth', 0.02,  'FaceColor', 'k', 'Normalization', 'probability')
    histogram(com_67/2800, 'BinWidth', 0.02, 'FaceColor',  [0 0.4470 0.7410], 'Normalization', 'probability')
